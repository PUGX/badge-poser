...
class KernelExceptionListenerTest extends PackagistWebTestCase
{
     ...
     * @dataProvider fakes
     */
    public function testOnErrorShouldCreateABadge($packagistClient)
    {
        $client = static::createClient();
        static::$kernel->getContainer()->set('packagist_client', $packagistClient);
        $client->request('GET', '/pugx/badge-poser/d/total');

        $this->assertTrue($client->getResponse()->isServerError(), $client->getResponse()->getContent());
        $this->assertStringStartsWith('<svg', $client->getResponse()->getContent(), $client->getResponse()->getContent());
    }
}
